...
if [ ! -f "$THUMBNAIL_FILE" ]; then
    echo "Warning: Thumbnail file '$THUMBNAIL_FILE' not found. Uploading without thumbnail."
    THUMBNAIL_PARAM=""
else
    THUMBNAIL_PARAM="-F \"thumbnail=@$THUMBNAIL_FILE\""
fi
...
CURL_CMD="curl -s -X POST \"$BASE_URL/api/videos\" \
  -H \"Authorization: Bearer $TOKEN\" \
  -F \"title=Test Video $(date +%s)\" \
  -F \"description=Automated test video upload\" \
  -F \"isPublic=true\" \
  -F \"video=@$VIDEO_FILE\" $THUMBNAIL_PARAM"
